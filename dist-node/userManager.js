"use strict";
/**
 * 用户管理器
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.userManager = exports.UserManager = void 0;
const utils_1 = require("./utils");
class UserManager {
    users = new Map();
    socketToUser = new Map();
    /**
     * 创建新用户 (Node.js version)
     */
    createUser(ws, username) {
        const userId = (0, utils_1.generateId)();
        const user = {
            id: userId,
            username: username.trim().slice(0, 20), // 限制用户名长度
            socketId: ws.id,
            roomId: null,
            color: (0, utils_1.generateUserColor)(),
            lastActive: (0, utils_1.now)(),
            mousePosition: null,
        };
        this.users.set(userId, user);
        this.socketToUser.set(ws.id, userId);
        ws.userId = userId;
        (0, utils_1.log)('info', `用户创建: ${user.username} (${userId})`);
        return user;
    }
    /**
     * 创建新用户 (Cloudflare Worker version)
     */
    createCloudflareUser(userId, username) {
        const user = {
            id: userId,
            username: username.trim().slice(0, 20), // 限制用户名长度
            socketId: userId,
            roomId: null,
            color: (0, utils_1.generateUserColor)(),
            lastActive: (0, utils_1.now)(),
            mousePosition: null,
        };
        this.users.set(userId, user);
        (0, utils_1.log)('info', `用户创建: ${user.username} (${userId})`);
        return user;
    }
    /**
     * 获取用户
     */
    getUser(userId) {
        return this.users.get(userId);
    }
    /**
     * 通过socket ID获取用户
     */
    getUserBySocket(socketId) {
        const userId = this.socketToUser.get(socketId);
        if (!userId)
            return undefined;
        return this.users.get(userId);
    }
    /**
     * 更新用户鼠标位置
     */
    updateMousePosition(userId, position) {
        const user = this.users.get(userId);
        if (!user)
            return false;
        user.mousePosition = position;
        user.lastActive = (0, utils_1.now)();
        return true;
    }
    /**
     * 更新用户最后活跃时间
     */
    updateLastActive(userId) {
        const user = this.users.get(userId);
        if (user) {
            user.lastActive = (0, utils_1.now)();
        }
    }
    /**
     * 删除用户
     */
    removeUser(userId) {
        const user = this.users.get(userId);
        if (!user)
            return undefined;
        this.socketToUser.delete(user.socketId);
        this.users.delete(userId);
        (0, utils_1.log)('info', `用户移除: ${user.username} (${userId})`);
        return user;
    }
    /**
     * 删除用户 (alias for Cloudflare compatibility)
     */
    deleteUser(userId) {
        return this.removeUser(userId);
    }
    /**
     * 通过socket删除用户
     */
    removeUserBySocket(socketId) {
        const userId = this.socketToUser.get(socketId);
        if (!userId)
            return undefined;
        return this.removeUser(userId);
    }
    /**
     * 检查用户名是否有效
     */
    validateUsername(username) {
        if (!username || typeof username !== 'string') {
            return { valid: false, error: '用户名不能为空' };
        }
        const trimmed = username.trim();
        if (trimmed.length === 0) {
            return { valid: false, error: '用户名不能为空' };
        }
        if (trimmed.length > 20) {
            return { valid: false, error: '用户名不能超过20个字符' };
        }
        // 检查非法字符
        if (!/^[\u4e00-\u9fa5a-zA-Z0-9_\-\s]+$/.test(trimmed)) {
            return { valid: false, error: '用户名包含非法字符' };
        }
        return { valid: true };
    }
    /**
     * 获取所有用户统计
     */
    getStats() {
        return {
            totalUsers: this.users.size,
        };
    }
    /**
     * 清理不活跃用户（可选的定时任务）
     */
    cleanupInactiveUsers(maxInactiveTime = 30 * 60 * 1000) {
        const currentTime = (0, utils_1.now)();
        let cleaned = 0;
        for (const [userId, user] of this.users.entries()) {
            if (currentTime - user.lastActive > maxInactiveTime) {
                this.removeUser(userId);
                cleaned++;
            }
        }
        if (cleaned > 0) {
            (0, utils_1.log)('info', `清理了 ${cleaned} 个不活跃用户`);
        }
        return cleaned;
    }
}
exports.UserManager = UserManager;
// 导出单例
exports.userManager = new UserManager();
